...
from tests.integration.aux.database_fixtures import (
    database_manager,
    clean_database,
    postgres_enhanced_client
)
...
class TestPostgreSQLPersistence:
    ...
    async def test_session_persists_after_login(self):
        ...
        user = await self.client.create_test_user_directly(
    ...
    async def test_multiple_users_isolation(self):
        ...
        for (email, _), expected_id in zip(users, user_ids):
    ...
    async def test_concurrent_operations_database_consistency(self):
        ...
        for i in range(5):
            user = await self.client.get_user_from_database(f"concurrent{i}@example.com")
            assert user is not None
...
class TestPostgreSQLPerformance:
    ...
    async def test_token_validation_performance(self):
        ...
        print(f"100 token validations in {elapsed:.2f} seconds ({elapsed/100:.3f}s per validation)")