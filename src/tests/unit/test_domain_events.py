...
from heimdall.domain.events import (
    DomainEvent, UserCreated, UserLoggedIn, UserLoggedOut,
    UserPasswordChanged, UserPermissionGranted, UserPermissionRevoked,
    UserDeactivated, UserActivated
)
from heimdall.domain.value_objects import Email, UserId, SessionId
...
class TestUserEvents:
    ...
    def test_event_serialization(self):
        ...
        assert isinstance(data["occurred_at"], str)
        datetime.fromisoformat(data["occurred_at"].replace('Z', '+00:00'))  # Should not raise
    ...
    def test_multiple_events_have_different_ids(self):
        ...
        assert event1.occurred_at <= event2.occurred_at  # Second event should be later or same time